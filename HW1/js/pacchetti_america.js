fetch('http://localhost/HW1/php/caricamento_america.php').then(onResponse).then(onJson);
...
function onJson(json) {
	...
	for (i in json) {
		...
		img2.src = 'http://localhost/HW1/foto/carrello/carrello++.png';
	...
	function selectCarrello(event) {
		...
		fetch('http://localhost/HW1/php/aggiungi_pacchetto.php', { method: 'POST', body: formData })
			.then(id_add_response)
			.then(id_add_json);
		...
		event.currentTarget.src = 'http://localhost/HW1/foto/carrello/carrello++.png';
		event.currentTarget.addEventListener('click', deselectCarrello);
		event.currentTarget.removeEventListener('click', selectCarrello);
	}
